...
bool is_full_binay_tree(struct tree_desc * head)
{
	if (head == NULL) {
		return true;
	}

	return process(head).b_full;
}

struct info process_cmp(struct tree_desc *x)
{
	struct info rep = { true, 0, 0, };
	if (x == NULL) {
		return rep;
	}

	struct info l_info = process_cmp(x->left);
	struct info r_info = process_cmp(x->right);

	int height = std::max(l_info.height, r_info.height) + 1;
	int nodes = l_info.nodes + r_info.nodes + 1;
...
bool is_full_binay_tree_cmp(struct tree_desc * head)
{
	if (head == NULL) {
		return true;
	}

	struct info rep = process_cmp(head);

	return (1 << rep.height) - 1 == rep.nodes;
}
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < times; i++) {
		...
		if (is_full_binay_tree(head) != is_full_binay_tree_cmp(head)) {
			printf("oops\n");
			succeed = false;
			break;
		}
