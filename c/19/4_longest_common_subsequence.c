 ...
 */
int process_vio(std::string str1, std::string str2, int i, int j)
{
	if (i == 0 && j == 0) {
		/*
		...
		if (str1[i] == str2[j]) {
			...
			return process_vio(str1, str2, i, j - 1);
		}
	} else if (j == 0) {
		/*
		 * 和上面的else if同理
		...
		if (str1[i] == str2[j]) {
			...
			return process_vio(str1, str2, i - 1, j);
		}
	} else {		/* i != 0 && j != 0 */
		/*
		 * 这里的情况为:
		 ...
		 * p1就是可能性c)
		 */
		int p1 = process_vio(str1, str2, i - 1, j);
		/* p2就是可能性b) */
		int p2 = process_vio(str1, str2, i, j - 1);
		/*
		 * p3就是可能性d), 如果可能性d)存在, 即str1[i] == str2[j], 那么p3就求出来, 参与pk
		 * 如果可能性d)不存在, 即str1[i] != str2[j], 那么让p3等于0, 然后去参与pk, 反正不影响
		 */
		int p3 = str1[i] == str2[j] ? (1 + process_vio(str1, str2, i - 1, j - 1)) : 0;

		return std::max(p1, std::max(p2, p3));
	}
}

int longest_common_subsequence_vio(std::string str1, std::string str2)
{
	if (str1.length() == 0 || str2.length() == 0) {
		return 0;
	}

	return process_vio(str1, str2, str1.length() - 1, str2.length() - 1);
}
 ...
 *
 *              x 位置一定 >= v 位置数值,
 *              因以 x 为中心时, 其值为左斜上, 左, 上为假设的最大值
 *
 */
int longest_common_subsequence_dp(std::string s1, std::string s2)
{
...
{
	std::string str1 = "a12c3";
	std::string str2 = "1c2ef3";

	printf("%d\n", longest_common_subsequence_vio(str1, str2));
	printf("%d\n", longest_common_subsequence_dp(str1, str2));
