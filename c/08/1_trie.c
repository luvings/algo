...
int prefix_number(m_trie_t *trie, std::string pre)
{
	if (pre.empty()) {
		return 0;
	}
	...
	int path = 0;
	for (uint32_t i = 0; i < pre.length(); i++) {
		path = chs[i] - 'a';
		if (node->nexts[path] == NULL) {
			return 0;
		}
...
void delete_un(m_trie_t *trie, std::string word)
{
	...
	for (uint32_t i = 0; i < word.length(); i++) {
		...
		struct trie_node *cur = prev->nexts[path];
		--cur->pass;

		if (cur->pass == 0) {
			struct trie_node *next = cur->nexts[chs[i + 1] - 'a'];

			prev->nexts[path] = NULL;
			prev->nexts[chs[i + 1] - 'a'] = next;
...
typedef struct trie_node1 {
	...
	int end;
	 ...
	 std::unordered_map < int, void *>nexts;
} m_node1_t;
...
bool contains(std::unordered_map < int, void *>nexts, int x)
{
...
void insert(m_trie1_t *trie, std::string word)
{
	...
	for (uint32_t i = 0; i < word.length(); i++) {
		...
		if (!contains(node->nexts, path)) {
			struct trie_node1 *t = (struct trie_node1 *)malloc(sizeof(struct trie_node1));
			// FIXME: ? ?
			memset((void *)t, 0, sizeof(struct trie_node1));
			std::unordered_map < int, void *>__map;
			__map.clear();
			t->nexts = __map;
		...
		node = (struct trie_node1 *)node->nexts.at(path);
		if (node)
			node->pass++;
	}
...
void delete_un(m_trie1_t *trie, std::string word)
{
	...
	for (uint32_t i = 0; i < word.length(); i++) {
		...
		struct trie_node1 *cur = (struct trie_node1 *)prev->nexts[(int)chs[i]];
		--cur->pass;

		if (cur->pass == 0) {
			struct trie_node1 *next = (struct trie_node1 *)cur->nexts[(int)chs[i + 1]];

			prev->nexts.erase(prev->nexts.find((int)chs[i]));
			prev->nexts.insert(std::make_pair((int)chs[i + 1], next));
			continue;
		}
