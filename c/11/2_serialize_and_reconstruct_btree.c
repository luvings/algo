...
void pre_order_s(struct tree_desc *head, std::queue < std::string > &q)
{
	if (head == NULL) {
		q.push("#");
	} else {
		...
		pre_order_s(head->left, q);
		pre_order_s(head->right, q);
	}
}

std::queue < std::string > pre_serialize(struct tree_desc *head)
{
...
struct tree_desc *pre_order_b(std::queue < std::string > &q)
{
	...
	head->left = pre_order_b(q);
	head->right = pre_order_b(q);

	return head;
}

struct tree_desc *build_tree_by_pre_q(std::queue < std::string > q)
{
	if (q.size() == 0) {
		return NULL;
	}

	return pre_order_b(q);
}
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < times; i++) {
		...
		std::queue < std::string > pre_s = pre_serialize(head);
		...
		struct tree_desc *pre_h = build_tree_by_pre_q(pre_s);
		struct tree_desc *post_h = build_tree_by_post_q(post_s);
		struct tree_desc *level_h = build_tree_by_level_q(level_s);

		if (!is_same_btree(pre_h, post_h) || !is_same_btree(pre_h, level_h)) {
			printf("oops\n");
			succeed = false;
			break;
		}
